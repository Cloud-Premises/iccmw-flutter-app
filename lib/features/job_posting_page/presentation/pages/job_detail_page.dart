...
class jobDetailPageState extends State<jobDetailPage> {
  ...
  Future<void> fetchjobData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final jobs = data['jobPosting'] as List<dynamic>;
  ...
  void _initializeFormControllers() {
    ...
    final data = jobData!['data'] as List<dynamic>;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          jobData != null ? jobData!['title'] ?? '' : 'job Detail',
  ...
  List<Widget> _buildjobDetails() {
    List<Widget> widgets = [];

    // Display title and description if available
    if (jobData!['title'] != null && jobData!['title'].isNotEmpty) {
      widgets.add(Text(
        jobData!['title'],
    ...
    if (jobData!['description'] != null && jobData!['description'].isNotEmpty) {
      widgets.add(Text(
        jobData!['description'],
